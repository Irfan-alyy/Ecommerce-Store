...
const FeaturedProducts = ({ quickView }) => {
  ...
  const handleAddCart = (product) => {
    ...
    if (!product?.variants?.[0]) {
      toast.error("Product variant unavailable", { position: "top-center" });
      return;
    }
    const item = {
      id: product.id,
    ...
    <>
      ...
      <div className="flex items-center flex-wrap gap-10 justify-center">
        {showProducts.map((elem, ind) => {
          const variant0 = elem?.variants?.[0];
          if (!variant0) return null; // skip rendering if no variant

          return (
            <div
              key={ind}
              className="poduct-card mb-[35px] flex flex-col w-[-260px] relative cart-flex transform transition-all ease-in"
            >
              ...
              >
                <img
                  src={`${BASE_URL}${variant0.images?.[0] ?? ""}`}
                  alt=""
                  loading="lazy"
                  className="w-full hover-image1 absolute inset-0 object-cover transition transform duration-500 ease-in-out m-auto"
                />
                <img
                  src={`${BASE_URL}${
                    elem?.variants?.[1]?.images?.[0] ?? variant0.images?.[0] ?? ""
                  }`}
                  loading="lazy"
                  alt=""
                  className="hover-image2 absolute inset-0 object-cover transition transform duration-500 ease-in-out m-auto"
                />
                <CiShoppingCart
                  title="Add Cart"
              ...
              <div className="flex justify-between mt-[20px] gap-1">
                <div>
                  ...
                  <p>
                    <span>$ {variant0.price}</span> -{" "}
                    <strike className="text-[rgb(127,127,127)]">
                      {variant0.discount
                        ? (variant0.price / (1 - variant0.discount / 100)).toFixed(2)
                        : "N/A"}
                    </strike>
                  </p>
                </div>
